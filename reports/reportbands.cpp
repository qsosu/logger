...
ReportBands::ReportBands(QSqlDatabase db, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ReportBands)
{
    ui->setupUi(this);
    this->db = db;
    ...
    if (query.exec("SELECT UPPER(BAND) AS BAND, COUNT(*) FROM records GROUP BY UPPER(BAND);"))
    {
        while (query.next()) {
...
void ReportBands::on_CloseButton_clicked()
{
    close();
}
...
void ReportBands::printBandsReport(QTableView *tableView, QWidget *parent)
{
    ...
    for (int startRow = 0; startRow < totalRows; startRow += rowsPerPage) {
        ...
        for (int i = 0; i < rowsOnPage; ++i) {
            ...
            // BAND (всегда верхний регистр)
            QString band = model->index(row, 0).data().toString().toUpper();
            painter.drawRect(x, y, colBandWidth, rowHeight);
            painter.drawText(QRect(x + 5, y, colBandWidth - 10, rowHeight),
                             Qt::AlignVCenter | Qt::AlignLeft, band);
            x += colBandWidth;
...
void ReportBands::on_PrintButton_clicked()
{
